...
import { ConfigureIdpFunc, IdpResp, IdpDetails } from '../types';
  ...
  implements Provider<ConfigureIdpFunc<IdpResp>>
{
  ...
  async configure(payload: IdpDetails): Promise<IdpResp> {
    ...
    try {
      ...
      const realmName = process.env.KEYCLOAK_REALM_NAME as string;
     ...
     // Create a new client within the realm
      ...
      const adminUsername = payload.email as string;
